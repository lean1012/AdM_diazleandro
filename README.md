...
Cortex A 
Esta serie está pensada para aplicaciones que tienen altos requisitos informáticos, ejecutan sistemas operativos de propósito general y pensado para manejar grande volúmenes de procesamiento y no tanto en restricciones de tiempo. Ejemplo un celular, en este queremos que tenga alto rendimiento el manejo de varias aplicaciones a la vez y no tanto que se tenga siempre la misma respuesta temporal.
...
Arquitecturas Load-Store solamente procesará datos que estén en sus propios registros y siempre se obtendrá el resultado en un registro. Este tipo de arquitecturas no tienen instrucciones que realicen operaciones de memoria a memoria. Las únicas operaciones que se aplican a la memoria son copiar datos de la memoria a los registros y de registros a memoria.

4. ¿Cómo es el mapa de memoria de la familia?

El mapa de memoria del cortex es un mapa de 4GB continuo de direcciones donde se mapea la memoria ram, la memoria flash y los periféricos. Esto hace que sea muy fácil acceder en C tanto a la memoria ram, periféricos, etc. a través de punteros, ya que todo se accede basandose en una dirección de 32bits.

5. ¿Qué ventajas presenta el uso de los “shadowed pointers” del PSP y el MSP?

Los microcontroladores disponen de un registro llamado SP, stack pointer, puntero a una sección de memoria RAM que es utilizada en forma de LIFO. EL PSP y MSP permiten manejar dos tipos de stack. El MSP es el puntero de stack por defecto, Main Stack Pointer, se usa en el modo Thread y siempre se usa en el modo Handler. En cambio, el PSP, Processor Stack Pointer, es usado por las tareas cuando se ejecutan en modo no privilegiado. En general se va a utilizar el MSP si se tiene un programa sencillo sin nigun sistema de operativo.
...
La excepción de reset es la que se produce cuando ocurre un reset. La excepción de NMI es una excepción producida por una interrupcion no enmascarable y la excepcion Hardfault es cuándo ocurre una falla, por ejemplo un acceso incorrecto a un espacio de memoria. Estas tienen diferenes prioridades, reset es la de mayor prioridad (-3), luego le sigue NMI (-2) y por último Hardfault (-1). Menor número mayor prioridad.

10. Describa las funciones principales de la pila. ¿Cómo resuelve la arquitectura el llamado a funciones y su retorno?

La pila o stack tiene como prinicipales funciones pasar datos a funciones o subrutinas, guardar variables locales, guardar el estado del procesador, por ejemplo cuando se atiende una interrupción o se salta a una subrutina, el micro debe guardar su estado. Es una parte de memoria que trabaja en modo LIFO.
...
Cuando se realiza el reset, el PC se inicializa con la dirección 0x00000000. La primera línea de código, en la dirección 0x00000000 contiene el valor del SP (ejemplo 0x20008000), (MSB= 0x20008000). Luego la siguiente posición de memoria (0x000000004) está la dirección de memoria de la rutina que atiende el reset. El PC toma ese valor y el programa salta a ese valor. En esta rutina estará la inicialización del microntrolador. Luego finalmente está la ubicación del programa principal, el main, se salta a ese valor y el microncontroaldor sigue con nuestro programa.


12. ¿Qué entiende por “core peripherals”? ¿Qué diferencia existe entre estos y el resto de los periféricos?

Los "core-peripherals" son los que están en el procesador diseñado por ARM. Estos son el controlador de interrupción NVIC, SysTick Timer y la MPU. El resto de los periféricos los va a poner los distintos fabricantes.

13. ¿Cómo se implementan las prioridades de las interrupciones? Dé un ejemplo

Si tenemos dos interrupciones al mismo tiempo, o una interrupción mientras se está ejecutando la rutina de otra interrupción, las prioridades van a definir como se sigue el procesamiento. Si interrumpe a la vez una interrupción de mayor prioridad y otra de menor, se atenderá a la de mayor interrupción. Si se está ejecutando el código de una interrupción y se genera una interrupción de mayor prioridad, se dejará de ejecutar la isr de la interrupción de menor prioridad para atender a la de mayor prioridad. La prioridad se representa con un valor numérico, cuanto menor sea el número ese número mayor prioridad tendrá.

El encargado de gestionar las interrupciones es el módulo NVIC, además nos permite asignar prioridad a nuestras interrupciones, deshabilitaras y habilitarlas ( a exepción del NMI, reset y hardfault). Estas últimas 3 son las únicas con prioridades fijas, (-3, -2 y -1 respectivamente). Las demás son configurables con prioridades desde el 0 al 255.


14. ¿Qué es el CMSIS? ¿Qué función cumple? ¿Quién lo provee? ¿Qué ventajas aporta?

El CMSIS es una librería provista por ARM que permiten usar funcionalidades estandar de Cortex, esto hace que a la hora de programar utilizando CMSIS es que es muy fácil portar el programa a otro Cortex.

15. Cuando ocurre una interrupción, asumiendo que está habilitada ¿Cómo opera el microprocesador para atender a la subrutina correspondiente? Explique con un ejemplo

Cuando ocurre una interrupción el micro entra en modo "exception sequence" donde preservará el estado, guardando los registros R0 al R3, R12, LR, xPSR y la dirección de retorno, si se usó punto flotante se guardarán todos sus registros. Luego por el bus de dato, él recibe el vector de interrupción que se utilizá para asociar una subrutina con determinada interrupción.

17. ¿Cómo cambia la operación de stacking al utilizar la unidad de punto flotante?

Al utilizar punto flotante, en el stack se preservan todos los registros relacionados con el punto flotante. Para eso el microcontrolador verifica el bit FPCA del registro de control, este nos informa si se ha utilizado la unidad de punto flotante.

16. Explique las características avanzadas de atención a interrupciones: tail chaining y late arrival.

La caracteristica Tail Chaining indica que el procesador no debe realizar un cambio de contexto para atender interrupciones seguidas. Por lo general, al salir de una interrupción el micro debe retaurar los estados para continuar, pero si hay otra excepción se tendría que preservar los estados otra vez. Esta caracteristica nos permite ahorrar ese paso.

Late Arrival nos permite eliminar las demoras adicionales cuando una interrupción de mayor prioridad ocurre mientras se está produciendo el cambio de contexto hacia una de menor prioridad. El micro es capaz de detectar una interrupción de mayor prioridad cuando se encuentra en la fase inicial de la interrupción, Stacking.

17. ¿Qué es el systick? ¿Por qué puede afirmarse que su implementación favorece la portabilidad de los sistemas operativos embebidos?

El Systick es el periférico que se utiliza en muchos de los sistemas operativos para llevar una base de tiempo. 
...
PendSV es una excepción que tiene la prioridad más baja, y se utiliza para realizar el cambio de contexto, por ejemplo en un sistema operativo para ir de una tarea a otra.
...
Los sufijos nos permiten agregar funcionalidad a las instrucciones. Existen sufijos que nos permiten operar con 8 y 16 bits. Ejemplo: ldrh esta instrucción cargar en un registro 16bits del contenido de una memoria. Si no se le pone sufijo está implícito que se trabaja con 32bits.


2. ¿Para qué se utiliza el sufijo ‘s’? Dé un ejemplo

El sufijo s es para indicar que se modifiquen las banderas al realizar una operación aritmética. En caso de que no se agregue, la operación no afectaran los flags, excepto la instrucción de cmp.
